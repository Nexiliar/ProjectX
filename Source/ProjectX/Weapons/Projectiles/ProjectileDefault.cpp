...
void AProjectileDefault::InitProjectile(FProjectileInfo InitParam)
{
	...
	if (InitParam.BulletMesh)
	{
		...
		BulletMesh->SetRelativeTransform(InitParam.BulletMeshTransform);
	}
	else
	...
	if (InitParam.BulletFX)
	{
		...
		BulletFX->SetRelativeTransform(InitParam.BulletFXTransform);
	}
	else
...
void AProjectileDefault::BulletCollisionSphereHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	if (OtherActor && Hit.PhysMaterial.IsValid())
	{
		...
		if (ProjectileSetting.HitDecals.Contains(mySurfacetype))
		{
			...
			{
				UGameplayStatics::SpawnDecalAttached(myMaterial, ProjectileSetting.DecalSize, OtherComp, NAME_None, Hit.ImpactPoint, Hit.ImpactNormal.Rotation(), EAttachLocation::KeepWorldPosition, ProjectileSetting.DecalLifetime);
			}
		}
